...
Här är den aktuella poängställningen efter **de första 2** av de totalt 5 deltävlingarna:
...
# Add ranking
res_1to2 <- res_1to2 %>% mutate(rank = dense_rank(desc(sum_points)))

res_1to2 <- res_1to2 %>% relocate(rank, .before = Name)


# Rename columns for final table
df <- res_1to2 %>% rename("#" = "rank", "Namn" = "Name", "Summa Poäng" = "sum_points")
